...
jQuery(function ($) {
  ...
  jQuery(document).ready(function ($) {
    // Fungsi untuk mendapatkan isi tabel dan mengonversinya ke format CSV
    function convertTableToCSV() {
      var csv = [];
      var rows = $('#the-list').find('tr:not(.no-items)');
    ...
    // Fungsi untuk mengunduh file CSV saat tombol diklik
    function downloadCSV() {
      var csv = convertTableToCSV();
      var blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
      var filename = 'data_tahfiz' + (Math.floor(Math.random() * (999 - 111 + 1)) + 1)+'.csv';

      if (navigator.msSaveBlob) { // Untuk Internet Explorer
    ...
    // Tambahkan tombol ekspor pada halaman post type 'tahfiz'
    var exportButton = $('<button class="button">Export CSV</button>');
    $('#post-query-submit').after(exportButton);

    // Event listener untuk mengunduh file CSV saat tombol diklik
    exportButton.on('click', function () {
      downloadCSV();
    });
  });

  // Fungsi untuk menangani perubahan pada elemen #select2-siswa-container
  ...
  // since 2.8 ajaxurl is always defined in the admin header and points to admin-ajax.php
    jQuery.post(obj.ajax_url, data, function (response) {
      ...
      console.log(json);
    // Set nilai dari elemen #nis dengan nilai teks yang dipilih
      $("#nis").val(json.nis);
      $("#kelas").val(json.kelas);
  });
  ...
  function handleImportSiswa(event) {
    ...
    reader.onload = function (event) {
      $("#result").append(
        `
        <div class="progress">
          <div class="progress-bar progress-bar-striped progress-bar-animated progressimport" role="progressbar" style="width: 0%" aria-valuenow="0" aria-valuemin="0" aria-valuemax="` +
          datas("length") +
          `"></div>
        </div>
      ...
      function importBy(index) {
        if (index  < datas("length") + 1) {
          let realKeys = datas(0);
          let keys = realKeys.map((word) =>
            word.replace(/ /g, "_").toLowerCase()
          );
          console.log(keys);
          ...
          if (
            numberRegex.test(assocDatas["nis"]) &&
            assocDatas["nama_lengkap"] != ""
          ) {
            ...
            $(".detail-progress").text(
              "Mengimport " + index + " dari " + datas("length") + " data."
            );
          } else {
            $(".logs").prepend("Data " + index + " tidak valid!\r\n");
          }
        } else if (index >= datas("length")) {
          $(".progressimport").removeClass(
          ...
          if (numberRegex.test(value[0])) {
            ...
            $("#result").append(
              '<div class="data-' +
                ...
                '">Import data ' +
                value[0] +
                "  <span>diprosses!</span></div>"
            );
            jQuery.post(obj.ajax_url, dataspp, function (response) {
              $(".data-" + response.nis + " span").html(
                '<span style="color:green;">' + response.status + "!</span>"
              );
              setTimeout(function () {
            ...
            $("#result").append(
              '<div class="data-' + value[0] + '">Data Tidak Valid!</div>'
            );
            setTimeout(function () {
              $(".data-" + value[0]).remove();
            }, 1000);
