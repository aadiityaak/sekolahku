...
function single_form()
{
    ...
    $title = isset($_POST['nis']) ? $_POST['nis'] : '';
    $attachment_id = '';
    $s = isset($_GET['cari']) ? $_GET['cari'] : '';
    print_r($_POST);
    if (!empty($_FILES) && wp_verify_nonce($_POST['_wpnonce'], 'submit_data')) {
        // add to post type document
        $rand = rand(11111, 99999);
        $post_id = wp_insert_post(array(
            'post_title' => $title . '-' . $rand,
            'post_content' => '',
            'post_status' => 'private',
            'post_type' => 'document',
            'post_author' => $user_id,
        ...
        echo '<div class="alert alert-warning">Ada kesalahan</div>';
    }
                ...
                <div class="modal-body">
                    <form method="post" action="" enctype="multipart/form-data">
                        <div class="mb-3">
                            ...
                            <?php wp_nonce_field('submit_data'); ?>
                            <input name="nis" class="form-control w-75" type="text" id="nis" placeholder="6572" required>
                        </div>
                        <div class="mb-3">
                            <label for="privasi" class="form-label">Privasi</label>
                            <select name="privasi" id="privasi" class="form-control w-75">
